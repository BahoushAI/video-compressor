...
export function drawMiniChart(canvas, points, trend) {
  ...
  for (let i = 1; i < pts.length; i++) {
    ...
    const y1 = h - 4 - ((pts[i - 1] - min) / (max - min)) * (h - 8);

    const x2 = 3 + (i / (pts.length - 1)) * (w - 6);
    const y2 = h - 4 - ((pts[i] - min) / (max - min)) * (h - 8);

    ctx.beginPath();
    ctx.moveTo(x1, y1);
    ctx.lineTo(x2, y2);

    ctx.strokeStyle = pts[i] > pts[i - 1] ? "#20d66b" :
                      pts[i] < pts[i - 1] ? "#ff3b30" :
                      "#20d66b";
  ...
  const lastX = w - 3;
  const lastY = h - 4 - ((last - min) / (max - min)) * (h - 8);

  ctx.beginPath();
  ctx.arc(lastX, lastY, 3, 0, Math.PI * 2);
  ctx.fillStyle = last >= previous ? "#20d66b" : "#ff3b30";
  ctx.fill();
}
